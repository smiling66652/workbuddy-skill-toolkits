...
// --- SQLite 执行：优先系统 sqlite3 命令，回退到 Node 22+ 内置 node:sqlite ----
// Windows 通常没有 sqlite3 命令，所以回退路径是必需的（否则 History 检索直接用不了）。
let _DatabaseSync;
function loadNodeSqlite() {
  if (_DatabaseSync !== undefined) return _DatabaseSync;
  _DatabaseSync = null;
  const orig = process.emitWarning;
  process.emitWarning = () => {};   // 屏蔽 node:sqlite 的 ExperimentalWarning，保持输出干净
  try {
    _DatabaseSync = createRequire(import.meta.url)('node:sqlite').DatabaseSync || null;
  } catch { /* 低于 Node 22 时没有该模块 */ } finally {
    process.emitWarning = orig;
  }
  return _DatabaseSync;
}

function queryRows(tmp, sql) {
  // 1) 系统 sqlite3
  try {
    ...
    return raw.trim().split('\n').filter(Boolean).map((line) => {
      ...
      return { title, url, visit, visit_count: parseInt(visit_count, 10) };
    });
  } catch (e) {
    if (e.code !== 'ENOENT') throw e;   // 命令存在但执行失败 → 真错误，向上抛
  }
  // 2) 回退：node:sqlite（Node 22+ 内置，零外部依赖）
  const DatabaseSync = loadNodeSqlite();
  if (!DatabaseSync) {
    die('既没有 sqlite3 命令，也没有 Node 内置 node:sqlite（需 Node 22+）。');
  }
...
function searchHistory(profileDir, profileName, browserLabel, keywords, since, limit, sort) {
  ...
  try {
    ...
    return queryRows(tmp, sql).map((r) => ({ browser: browserLabel, profile: profileName, ...r }));
  } catch (e) {
    return [];
  } finally {
