...
> ⚠️ **先看这一段，能省掉全部安装步骤。**
>
> **图片和视频生成优先用 Agent 内置工具，不要走本页的 CLI：**
> - 文生图 / 图生图 → 内置 **ImageGen**
> - 文生视频 / 图生视频 → 内置 **VideoGen**（输出 MP4 到本地）
> - 文生 3D、图片模板特效 → 多模态生成技能包
>
> 内置工具零安装、无需 API key，是本层的**首选**。
>
> **本页的 MiniMax CLI（`mmx`）在本机未安装**，且需要 MiniMax API key。
> 只有在"内置工具不满足 + 你确实想用 MiniMax 音色/配额"时才装。
> 语音合成（TTS）是内置工具没覆盖的方向，这一项才是 `mmx` 的真正价值。
...
| 工具 | 速度 | 质量 | 费用 | 状态 |
|------|------|------|------|------|
| **内置 ImageGen / VideoGen** | 快 | 高 | 无 | ✅ **可用，首选** |
| **MiniMax CLI（mmx）** | 快 | 高 | 按量付费 | ❌ 未安装，需 key |
| OpenAI DALL-E | 中 | 非常高 | 按张付费 | ❌ 需 `OPENAI_API_KEY`（未设置） |
| OpenAI Whisper | 中 | 非常高 | 按分钟付费 | ❌ 需 key；转写可先用本地方案 |
| Stable Diffusion | 慢 | 高 | 本地免费 | ❌ 本机未部署 |
